...
export default function App() {
  ...
  const [adjustMode, setAdjustMode] = useState<'none' | 'fill' | 'fit' | 'stretch'>('stretch');
  ...
  // Update the handlePreviewAndDownload function in App.tsx
  const handlePreviewAndDownload = async () => {
    
    // if (images.length === 0 || !width || !height) return;
    if (images.length === 0 || (adjustMode !== 'none' && (!width || !height))) return;
    ...
    try {
      const { data, info } = await processImage(images[0].file, {
        ...
        // width: width || 0, // Width will be ignored for 'none' mode
        // height: height || 0, // Height will be ignored for 'none' mode
        unit: selectedUnit,
        mode: adjustMode,
        format,
        dpi: dpi || 96, // Default to 96 DPI if not set
